...
class PlayState extends MusicBeatState
{
	...
	public var modchartSounds:Array<FlxSound> = new Array<FlxSound>();
	#if (haxe >= "4.0.0")
	...
	var sabotageTiming:Array<Array<Int>>;
	var lastSabotageEvent:Int = -1;

	var sabotageGroup:FlxSpriteGroup;

	var sabotageWhiteText:FlxText;
	var sabotageFrame:FlxSprite;
	var sabotageText:FlxText;

	var sabotageAlarm:FlxSound;
	var sabotageBeats:Int = 0;

	var cafeteriaDancersY:Float = 640;
	var cafeteriaDancers:BGSprite;

	var cafeteriaGradient:BGSprite;
	var cafeteriaTables:BGSprite;

	var airshipDancers:BGSprite;
	...
	private var spinLength:Int = 6;
	private var bar:Int = 16;
	...
	override public function create()
	{
		...
		BGSprite.directory = stageData.directory;
		...
		switch (curStage)
		{
			...
			case 'cafeteria':
			{
				var parallax:Float = .25;

				var halfInverseParallax = 1 - (parallax / 2);
				var inverseParallax:Float = 1 - parallax;

				var space:BGSprite = new BGSprite('space', -600, -100, parallax, parallax);
				var bg:BGSprite = new BGSprite('background', -920);

				if (!ClientPrefs.lowQuality)
				{
					cafeteriaDancers = new BGSprite('sus', -600, cafeteriaDancersY, halfInverseParallax, halfInverseParallax, ['sus']);
					cafeteriaTables = new BGSprite('tables', -720, -100, inverseParallax, inverseParallax);

					cafeteriaGradient = new BGSprite('gradient', 0, 700, 0, 1);
					cafeteriaGradient.setGraphicSize(FlxG.width, Std.int(cafeteriaGradient.height));

					cafeteriaDancers.setGraphicSize(Std.int(cafeteriaDancers.width * .9));
					cafeteriaTables.setGraphicSize(Std.int(cafeteriaTables.width * 1.05));

					cafeteriaGradient.updateHitbox();
					cafeteriaDancers.updateHitbox();
					cafeteriaTables.updateHitbox();
				}

				space.setGraphicSize(Std.int(space.width * 1.05));
			...
			case 'airship':
			{
				var bg:BGSprite = new BGSprite('lol', -1000, -300);
				if (!ClientPrefs.lowQuality)
				{
					airshipDancers = new BGSprite('characters', -500, 300, 1, 1, ['bopawesome']);

					airshipDancers.setGraphicSize(Std.int(airshipDancers.width * .8));
					airshipDancers.updateHitbox();
				}
		...
		add(gfGroup);
		if (!ClientPrefs.lowQuality) { switch (curStage) { case 'airship': add(airshipDancers); } }
		...
		if (!ClientPrefs.lowQuality) { switch (curStage) { case 'cafeteria': { add(cafeteriaTables); add(cafeteriaDancers); add(cafeteriaGradient); } } }
	...
	private function bgDance(?sprite:BGSprite = null, beat:Int)
	{
		if (sprite != null && !ClientPrefs.lowQuality)
		{
			var curAnim:FlxAnimation = sprite.animation.curAnim;
			if (curAnim != null && (beat % gfSpeed) == 0) sprite.dance(true);
		}
	}
	...
	public function startCountdown():Void
	{
		...
		{
			var loopsLeft:Int = tmr.loopsLeft + 1;
			...
			switch (curStage)
			{
				case 'cafeteria': bgDance(cafeteriaDancers, loopsLeft);
				case 'airship': bgDance(airshipDancers, loopsLeft);
			}
			switch (tmr.loops - loopsLeft)
			{
				case 0: FlxG.sound.play(Paths.sound('intro3$introAssetsSuffix'), 0.6);
				case 1:
				{
					...
					add(countdownReady);
					modchartTweens.push(FlxTween.tween(countdownReady, {/*y: countdownReady.y + 100,*/ alpha: 0}, Conductor.crochet / 1000, {
					...
					FlxG.sound.play(Paths.sound('intro2$introAssetsSuffix'), 0.6);
				}
				case 2:
				{
					...
					add(countdownSet);
					modchartTweens.push(FlxTween.tween(countdownSet, {/*y: countdownSet.y + 100,*/ alpha: 0}, Conductor.crochet / 1000, {
						ease: FlxEase.cubeInOut,
						onComplete: function(twn:FlxTween)
						{
							remove(countdownSet);
					...
					FlxG.sound.play(Paths.sound('intro1$introAssetsSuffix'), 0.6);
				}
				case 3:
				{
					...
					add(countdownGo);
					modchartTweens.push(FlxTween.tween(countdownGo, {/*y: countdownGo.y + 100,*/ alpha: 0}, Conductor.crochet / 1000, {
						ease: FlxEase.cubeInOut,
						onComplete: function(twn:FlxTween)
						{
							remove(countdownGo);
					...
					FlxG.sound.play(Paths.sound('introGo$introAssetsSuffix'), 0.6);
				}
			}
	...
	function startSong():Void
	{
		...
		if (curStep < 1) stepHit();
		// Song duration in a float, useful for the time left feature
		songLength = FlxG.sound.music.length;

		modchartTweens.push(FlxTween.tween(timeBar, {alpha: 1}, 0.5, { ease: FlxEase.circOut, onComplete: function(twn:FlxTween) { modchartTweens.remove(twn); } }));
		modchartTweens.push(FlxTween.tween(timeTxt, {alpha: 1}, 0.5, { ease: FlxEase.circOut, onComplete: function(twn:FlxTween) { modchartTweens.remove(twn); } }));
	...
	function eventPushed(event:Array<Dynamic>) {
		switch (event[1]) {
			case 'Sabotage':
			{
				if (!ClientPrefs.lowQuality)
				{
					var beats:Int = Math.isNaN(event[2]) ? 0 : event[2];
					var timing:Int = event[0];

					if (lastSabotageEvent >= 0 && beats <= 0)
					{
						if (sabotageTiming == null) sabotageTiming = new Array<Array<Int>>();

						sabotageTiming.push([lastSabotageEvent, timing]);
						lastSabotageEvent = -1;
					}
					if (beats > 0) lastSabotageEvent = timing;
					if (sabotageGroup == null)
					{
						var height:Int = 200;
						var width:Int = 400;

						sabotageGroup = new FlxSpriteGroup(-(FlxG.width / 2) + width - ((ClientPrefs.middleScroll ? -(STRUM_X_MIDDLESCROLL + (STRUM_X * 2)) : (STRUM_X * 2))), ClientPrefs.downScroll ? (((FlxG.height / 2) - height) - ((timeBarBG.height + strumLine.height) * 2)) : (-((FlxG.height - height) / 2) + ((timeBarBG.height + strumLine.height) * 2)));
						sabotageGroup.cameras = [ camHUD ];

						sabotageFrame = new FlxSprite().makeGraphic(width, height, new FlxColor(0x7F808080));

						sabotageWhiteText = new FlxText();
						sabotageText = new FlxText();

						sabotageWhiteText.setFormat(Paths.font("arial.ttf"), 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
						sabotageText.setFormat(Paths.font("arial.ttf"), 24, FlxColor.YELLOW, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);

						sabotageWhiteText.borderQuality = 1;
						sabotageWhiteText.borderSize = 2;

						sabotageText.borderQuality = 1;
						sabotageText.borderSize = 2;

						sabotageFrame.updateHitbox();
						sabotageFrame.screenCenter();

						sabotageGroup.add(sabotageFrame);

						sabotageGroup.add(sabotageWhiteText);
						sabotageGroup.add(sabotageText);

						sabotageGroup.alpha = 0;
						add(sabotageGroup);
					}
				}
				if (sabotageAlarm == null)
				{
					var soundPath:String = Paths.sound('sabotageAlarm');

					sabotageAlarm = new FlxSound().loadEmbedded(soundPath);
					sabotageAlarm.volume = 1 / 2;

					CoolUtil.precacheRawSound(soundPath);
				}
			}
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		{
			// FlxG.log.add(i);
			var targetAlpha:Float = (player != 1 && ClientPrefs.middleScroll) ? .35 : 1;
			...
			else
			{
				...
				modchartTweens.push(FlxTween.tween(babyArrow, {y: babyArrow.y + 10, alpha: targetAlpha}, 1, { ease: FlxEase.circOut, startDelay: .5 + (.2 * i), onComplete: function(twn:FlxTween) { modchartTweens.remove(twn); } }));
			}
	...
	{
		quickUpdatePresence(null, false, true);
	...
	{
		quickUpdatePresence('PAUSED - ', false, true);
	...
	override public function update(elapsed:Float)
	{
		...
		else
		{
			...
			if (!paused)
			{
				...
				var curTime:Float = Math.max(Conductor.songPosition - ClientPrefs.noteOffset, 0);
				if (updateTime)
				{
					songPercent = curTime / songLength;

					var songCalc:Float = ClientPrefs.timeBarType == 'Time Elapsed' ? curTime : (songLength - curTime);
					var secondsTotal:Int = Math.floor(Math.max(songCalc / 1000, 0));

					if (ClientPrefs.timeBarType != 'Song Name') timeTxt.text = FlxStringUtil.formatTime(secondsTotal);
				}
				if (sabotageText != null && sabotageTiming.length > 0)
				{
					var latest:Array<Int> = sabotageTiming[0];
					var timeEnd:Int = latest[1];

					if (timeEnd < curTime) { sabotageTiming.shift(); }
					else
					{
						var timeStart:Int = latest[0];
						var offset:Float = 4;

						if (timeStart <= curTime)
						{
							var timeDiff:Float = Math.max((timeEnd + (offset * 1000)) - curTime, 0) / 1000;
							var height:Float = sabotageFrame.height;

							var sabotageTasks:String = switch (curStage)
							{
								case 'cafeteria': 'Storage: Fix Wiring (1/3)\nAdmin: Upload Data (1/2)\nFunkin: Beat Opponents (1/8)\n';
								default: '';
							}

							sabotageText.text = '${sabotageTasks}Reactor Meltdown in ${Math.floor(timeDiff)} (${Math.round(Math.max(((offset * 2) - (timeDiff - offset)) / offset, 0))}/2)\n';
							sabotageWhiteText.text = switch (curStage)
							{
								case 'cafeteria': 'Admin: Swipe Card\nO2: Clean O2 Filter\nCafeteria: Empty Garbage (0/2)\n';
								default: '';
							};

							sabotageWhiteText.updateHitbox();
							sabotageText.updateHitbox();

							sabotageWhiteText.screenCenter();
							sabotageText.screenCenter();

							var whiteTextHeight:Float = sabotageWhiteText.frameHeight / 2;

							sabotageWhiteText.y += sabotageGroup.y - ((height / 2) - whiteTextHeight);
							sabotageWhiteText.x += sabotageGroup.x - (STRUM_X / 2);

							sabotageText.y = sabotageWhiteText.y + (sabotageText.frameHeight / 2);
							sabotageText.x = sabotageWhiteText.x;

							// sabotageText.y += sabotageGroup.y - ((height / 2) - (sabotageText.frameHeight / 2));
							// sabotageText.x += sabotageGroup.x - (STRUM_X * height / sabotageFrame.width);
						}
					}
				}
			}
	...
	function tweenFunction(sprite:FlxSprite, value:Float) { sprite.alpha = value; }
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch (eventName) {
			...
			case 'Sabotage':
			{
				...
				var newValue:Int = Math.isNaN(value) ? 0 : Std.int(Math.max(value, 0));

				if (sabotageFrame != null)
				{
					var oldSign:Int = sabotageBeats <= 0 ? 0 : 1;
					var newSign:Int = newValue <= 0 ? 0 : 1;

					if (oldSign != newSign)
					{
						var crochet:Float = (Conductor.crochet / 1000) * 2;
						var isSabotaged:Bool = newSign > 0;

						modchartTweens.push(FlxTween.num(1 - newSign, newSign, crochet, { ease: FlxEase.quadOut, onComplete: function(twn:FlxTween) { modchartTweens.remove(twn); } }, tweenFunction.bind(sabotageGroup)));
						if (!ClientPrefs.downScroll)
						{
							var shifting:Int = Math.floor(opponentStrums.length / (ClientPrefs.middleScroll ? 2 : 1));
							for (index in 0...shifting) { var note:StrumNote = opponentStrums.members[index]; FlxTween.cancelTweensOf(note); modchartTweens.push(FlxTween.tween(note, { y: isSabotaged ? (sabotageFrame.y + Note.swagWidth + strumLine.height + (sabotageFrame.height / 2)) : strumLine.y }, crochet, { ease: FlxEase.quadOut, onComplete: function(twn:FlxTween) { modchartTweens.remove(twn); } })); }
						}
						if (!ClientPrefs.lowQuality)
						{
							switch (curStage)
							{
								case 'cafeteria':
								{
									FlxTween.cancelTweensOf(cafeteriaDancers);
									modchartTweens.push(FlxTween.tween(cafeteriaDancers, { y: isSabotaged ? (FlxG.height + cafeteriaDancers.frameHeight) : cafeteriaDancersY }, crochet, { startDelay: crochet * (isSabotaged ? 1 : 2), ease: isSabotaged ? FlxEase.backIn : FlxEase.backInOut, onComplete: function(twn:FlxTween) { modchartTweens.remove(twn); } }));
								}
							}
						}
					}
				}
				sabotageBeats = newValue;
			}
			case 'Change Strumline Visibility':
			{
				var visible:Bool = value1.toLowerCase().startsWith('true') || value1.trim().length <= 0;
				var beats:Float = Std.parseFloat(value2.trim());

				var instant:Bool = Math.isNaN(beats) || beats <= 0 || ClientPrefs.lowQuality;
				var alpha:Float = visible ? 1 : 0;

				switch (instant)
				{
					case true:
					{
						for (note in opponentStrums.members) note.alpha = alpha;
						for (note in playerStrums.members) note.alpha = alpha;
					}
					default:
					{
						for (note in opponentStrums.members)
						{
							...
							FlxTween.num(1 - alpha, alpha, (Conductor.crochet / 1000) * beats, { ease: FlxEase.linear }, tweenFunction.bind(note));
						}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if(!practiceMode && !cpuControlled) {
			...
			if(ClientPrefs.scoreZoom)
			{
				...
				modchartTweens.push(scoreTxtTween);
			}
		}
		...
		modchartTweens.push(FlxTween.tween(rating, {alpha: 0}, 0.2, {
			startDelay: Conductor.crochet / 1000,
			onComplete: function(twn:FlxTween) { modchartTweens.remove(twn); }
		}));
	...
	private function quickUpdatePresence(?startString:String = "", ?hasLength:Bool = true, ?disregardCPU = false)
	{
		#if desktop
		if (health > 0 && !paused && (disregardCPU || !cpuControlled)) DiscordClient.changePresence(detailsText, '$startString${getFormattedSong()}', iconP2.getCharacter(), hasLength && Conductor.songPosition > 0, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
		#end
	}
	...
	override function beatHit()
	{
		...
		if (sabotageBeats > 0 && (curBeat % sabotageBeats) == 0 && sabotageAlarm != null) { sabotageAlarm.play(true); if (ClientPrefs.flashing) camOther.flash(new FlxColor(0x4CFF0000)); }
		if (curBeat % gfSpeed == 0) {
			...
			modchartTweens.push(FlxTween.angle(iconP1, -angleValue, 0, Conductor.crochet / (crochetDiv * gfSpeed), { ease: FlxEase.quadOut, onComplete: function(twn:FlxTween) { modchartTweens.remove(twn); } }));
			modchartTweens.push(FlxTween.angle(iconP2, angleValue, 0, Conductor.crochet / (crochetDiv * gfSpeed), { ease: FlxEase.quadOut, onComplete: function(twn:FlxTween) { modchartTweens.remove(twn); } }));

			modchartTweens.push(FlxTween.tween(iconP1, {'scale.x': 1, 'scale.y': 1}, Conductor.crochet / (crochetDiv * gfSpeed), { ease: FlxEase.quadOut, onComplete: function(twn:FlxTween) { modchartTweens.remove(twn); } }));
			modchartTweens.push(FlxTween.tween(iconP2, {'scale.x': 1, 'scale.y': 1}, Conductor.crochet / (crochetDiv * gfSpeed), { ease: FlxEase.quadOut, onComplete: function(twn:FlxTween) { modchartTweens.remove(twn); } }));
		...
		switch (curStage)
		{
			case 'cafeteria': bgDance(cafeteriaDancers, curBeat);
			case 'airship': bgDance(airshipDancers, curBeat);
		}
		lastBeatHit = curBeat;
	}
